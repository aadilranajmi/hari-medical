import { v4 as uuid } from 'uuid';
import heroImage from '@/assets/images/banner-joint-pain.jpg';
export const homeData = {
    heroData: [
        {
            uid: uuid(),
            imgUrl: heroImage,
            heading: 'Suffering from Joint Pain?',
            para: 'Ayurveda has its permanent solution.',
            ctaLink: '/',
        },
        {
            uid: uuid(),
            imgUrl: heroImage,
            heading: 'Suffering from Joint Pain?',
            para: 'Ayurveda has its permanent solution.',
            ctaLink: '/',
        },
        {
            uid: uuid(),
            imgUrl: heroImage,
            heading: 'Suffering from Joint Pain?',
            para: 'Ayurveda has its permanent solution.',
            ctaLink: '/',
        },
        {
            uid: uuid(),
            imgUrl: heroImage,
            heading: 'Suffering from Joint Pain?',
            para: 'Ayurveda has its permanent solution.',
            ctaLink: '/',
        },
    ],
    treatmentWork: [
        {
            uid: uuid(),
            imgUrl: '/images/potli_sek_therapie.jpg',
            title: 'Ancient',
        },
        {
            uid: uuid(),
            imgUrl: '/images/potli_sek_therapie.jpg',
            title: 'Holistic Treatment',
        },
        {
            uid: uuid(),
            imgUrl: '/images/potli_sek_therapie.jpg',
            title: 'Cure From The Root',
        },
        {
            uid: uuid(),
            imgUrl: '/images/potli_sek_therapie.jpg',
            title: 'No Chemical Used',
        },
        {
            uid: uuid(),
            imgUrl: '/images/potli_sek_therapie.jpg',
            title: 'No Side Effect',
        },
        {
            uid: uuid(),
            imgUrl: '/images/potli_sek_therapie.jpg',
            title: 'Oldest Medicine  System',
        },
    ],
    welcomeHome: [
        {
            uid: uuid(),
            imgUrl: '/images/potli_sek_therapie.jpg',
            title: 'Authentic And Traditional Therapies',
        },
        {
            uid: uuid(),
            imgUrl: '/images/potli_sek_therapie.jpg',
            title: 'One Of A Kind Treatments',
        },
        {
            uid: uuid(),
            imgUrl: '/images/potli_sek_therapie.jpg',
            title: 'Natural Remedies',
        },
        {
            uid: uuid(),
            imgUrl: '/images/potli_sek_therapie.jpg',
            title: '30+ Years Of Experience',
        },
        {
            uid: uuid(),
            imgUrl: '/images/potli_sek_therapie.jpg',
            title: 'Natural Remedies',
        },
    ],
    services: [
        {
            uid: uuid(),
            imgUrl: '/images/potli_sek_therapie.jpg',
            title: 'Potli Sek Therapie',
        },
        {
            uid: uuid(),
            imgUrl: '/images/jal_chikitsa_therapie.jpg',
            title: 'Jal Chikitsa Therapie',
        },
        {
            uid: uuid(),
            imgUrl: '/images/teil_shirodhara.jpg',
            title: 'Teil Shirodhara',
        },
        {
            uid: uuid(),
            imgUrl: '/images/mitti_shirodhara.jpg',
            title: 'Mitti Shirodhara',
        },
        {
            uid: uuid(),
            imgUrl: '/images/vasti_therapie.jpg',
            title: 'Vasti Therapie',
        },
        {
            uid: uuid(),
            imgUrl: '/images/enema_panchakarma.jpg',
            title: 'Enema Panchakarma',
        },
        {
            uid: uuid(),
            imgUrl: '/images/acupressure_treatment.jpg',
            title: 'Acupressure Treatment',
        },
        {
            uid: uuid(),
            imgUrl: '/images/surya_chikitsa.jpg',
            title: 'Surya Chikitsa',
        },
        {
            uid: uuid(),
            imgUrl: '/images/ayurvedic_massage.jpg',
            title: 'Ayurvedic Massage',
        },
    ],
};
